...
fn template_rt() -> (Arc<Runtime>, tempfile::TempDir) {
    ...
    let loader = Arc::new(StaticLoader(json!({
        "mounts": [{ "path": "/render", "service": "template", "config": { "access": "open" } }]
    })));
    let rt = Runtime::new(
...
async fn put_template(rt: &Runtime, name: &str, source: &str) -> Option<StatusCode> {
    rt.handle(
        req(Method::PUT, &format!("/render/.templates/{name}"))
            .with_json(&json!({ "source": source })),
    )
    .await
    .status
}
...
async fn renders_from_json_body_and_query_params() {
    ...
    assert_eq!(
        put_template(&rt, "welcome", GREETER_V1).await,
        Some(StatusCode::CREATED)
    );

    // POST with a JSON body → props.
    let mut resp = rt
        .handle(req(Method::POST, "/render/welcome").with_json(&json!({ "name": "Alice" })))
        .await;
    ...
    assert!(
        text(&mut resp).await.contains("Hello, Alice!"),
        "body greets Alice"
    );

    // GET with query params → props (same template, no body).
    let mut resp = rt
        .handle(req(Method::GET, "/render/welcome?name=Bob"))
        .await;
    assert_eq!(resp.status, Some(StatusCode::OK));
    assert!(
        text(&mut resp).await.contains("Hello, Bob!"),
        "query param becomes a prop"
    );

    // No props → the template's own default.
    let mut resp = rt.handle(req(Method::GET, "/render/welcome")).await;
    assert!(
        text(&mut resp).await.contains("Hello, world!"),
        "default when no props"
    );
}
...
async fn authoring_subtree_round_trips_and_lists() {
    ...
    let mut resp = rt
        .handle(req(Method::GET, "/render/.templates/welcome"))
        .await;
    assert_eq!(resp.status, Some(StatusCode::OK));
    let env: Value = serde_json::from_str(&text(&mut resp).await).expect("stored envelope is JSON");
    assert!(
        env["source"].as_str().unwrap().contains("Hello, "),
        "source round-trips"
    );
...
async fn new_version_replaces_the_cached_isolate() {
    ...
    assert!(
        body.contains("v2:Ada"),
        "the cached isolate was rebuilt for v2: {body}"
    );
...
async fn validator_rejects_an_envelope_without_source() {
    ...
    assert_eq!(
        ...
        "missing source is rejected at write"
    );
}
...
async fn unmatched_path_is_not_found() {
    let (rt, _dir) = template_rt();
    let resp = rt
        .handle(req(Method::POST, "/render/nope").with_json(&json!({})))
        .await;
    assert_eq!(
        resp.status,
