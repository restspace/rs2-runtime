...
async fn file_write_read_list_delete() {
    ...
    // PUT creates → 201
    let resp = rt
        .handle(req(Method::PUT, "/files/docs/a.txt").with_body(Body::from_string("alpha", MediaType::new("text/plain"))))
        .await;
    assert_eq!(resp.status, Some(StatusCode::CREATED));

    // PUT overwrites → 200
    let resp = rt
        .handle(req(Method::PUT, "/files/docs/a.txt").with_body(Body::from_string("alpha2", MediaType::new("text/plain"))))
        .await;
    ...
    rt.handle(req(Method::PUT, "/files/docs/b.txt").with_body(Body::from_string("beta", MediaType::new("text/plain"))))
        .await;
    let mut resp = rt.handle(req(Method::GET, "/files/docs/?$take=1&$skip=1")).await;
    assert_eq!(resp.status, Some(StatusCode::OK));
    assert_eq!(resp.body.as_ref().unwrap().media_type.essence(), "application/vnd.rs2.dir+json");
    ...
    assert_eq!(rt.handle(req(Method::DELETE, "/files/docs/a.txt")).await.status, Some(StatusCode::NO_CONTENT));
    assert_eq!(rt.handle(req(Method::DELETE, "/files/docs/b.txt")).await.status, Some(StatusCode::NO_CONTENT));
    assert_eq!(rt.handle(req(Method::DELETE, "/files/docs/")).await.status, Some(StatusCode::NO_CONTENT));
    assert_eq!(rt.handle(req(Method::GET, "/files/docs/a.txt")).await.status, Some(StatusCode::NOT_FOUND));
}
...
async fn path_traversal_is_rejected_at_the_router() {
    let dir = tempfile::tempdir().unwrap();
    let rt = test_runtime(dir.path());
    for path in ["/files/../secret", "/files/%2e%2e/secret", "/files/a%00.txt"] {
        let resp = rt.handle(req(Method::GET, path)).await;
        assert_eq!(resp.status, Some(StatusCode::BAD_REQUEST), "path {path} must be rejected");
    }
}
...
async fn data_crud_schema_validation_and_confirm_delete() {
    ...
    let resp = rt.handle(req(Method::PUT, "/data/people/.schema.json").with_json(&schema)).await;
    assert_eq!(resp.status, Some(StatusCode::OK));

    // Valid write → 201; invalid write → 422 with structured errors.
    let resp = rt.handle(req(Method::PUT, "/data/people/ada").with_json(&json!({ "name": "Ada" }))).await;
    assert_eq!(resp.status, Some(StatusCode::CREATED));
    let mut resp = rt.handle(req(Method::PUT, "/data/people/bad").with_json(&json!({ "age": 7 }))).await;
    ...
    assert_eq!(resp.status, Some(StatusCode::OK));
    assert_eq!(resp.body.as_ref().unwrap().media_type.schema(), Some("/data/people/.schema.json"));
    ...
    let resp = rt.handle(req(Method::POST, "/data/people").with_json(&json!({ "name": "Grace" }))).await;
    assert_eq!(resp.status, Some(StatusCode::CREATED));
...
async fn errors_are_problem_json_with_trace() {
    ...
    assert_eq!(resp.status, Some(StatusCode::NOT_FOUND));
    assert_eq!(resp.body.as_ref().unwrap().media_type.essence(), "application/problem+json");
    let problem = resp.body.as_mut().unwrap().as_json(65536).await.unwrap();
...
async fn tenant_storage_is_host_scoped() {
    ...
    let write = Message::request(Method::PUT, "/files/x.txt", "alpha")
        .with_body(Body::from_string("alpha-data", MediaType::new("text/plain")));
