...
| The `images` grant (`{"type": "images", "source": "<grant>", "cache": "<grant>"}` on a `code:` mount) is served only here | Both hosts accept the grant at build time so one tenant file mounts on both; a call answers **501** `provider_unavailable` on Rust (no transform backend) and on a Worker deployed without the `images` binding. The image mount is therefore `guest-services/image` (Wasm, codecs inside) on Rust and `guest-services/image-js` (JS over the grant) here — same URLs, params, config, headers, ETags and cache layout (`image.test.ts`; divergences `imageBundle`, `imageFidelity`) |
...
| — | `capabilities/images.ts` | new | The `images` grant target (§I decision 47): `GET /info?source=` and `GET /transform?source=&w=&h=&fit=&g=x,y&rect=&f=&q=&maxSourcePixels=[&store=]` in the RS2 image vocabulary, mapped onto the Images binding (`fit=fill` → `squeeze`, `g` fractions → `gravity: {x, y, mode: "remainder"}`, `rect` → `trim`). The source is read through the sibling `source` grant **under the caller's principal**; `store` writes the derivative through the sibling `cache` grant (no principal) and replies `{mediaType, bytes, stored}`; without `store` the reply is the bytes (the guest's `x-rs2-body-ref: images:/transform?…` nostore fallback). `info()` guards `maxSourcePixels` → 413 with the component's wording; undecodable/SVG → 415; failures are status responses, never guest throws. Rust `code.rs` accepts the grant shape and 501s at use. |
...
resident adapters and wait for P4b. `guest-services/image-js/image.js` is the
Worker's image-transform service: a dependency-free ESM bundle (pure-JS
SHA-256 — the Rust prelude has no `crypto.subtle`) that canonicalizes, keys
the derivative, and transforms **by reference** through the `images` grant.
...
| `RS2_IMAGE_COMPONENT` | Rust leg of `image.test.ts`: the built `guest-services/image` `.wasm` (the suite skips without it; start `host:rust` with `RS2_RUST_FEATURES=js,wasm`) |
| `RS2_CF_REMOTE` | marks a real-platform Worker run: enables the `@remote` cases (memory cap) and full-fidelity image geometry (`imageFidelity`) |
...
| `image.test.ts` | `image_service.rs` (HTTP-visible subset) | the `code:image@<v>` mount on both hosts — `$info`, passthrough + ETag revalidation, `w`/`h` scale-down with dimensions decoded from the PNG/JPEG headers (never encoded bytes), strong derived ETag + 304, `x-img-cache` miss→hit, dpr folding/param order sharing one derivative, HEAD, `f=jpeg`/`q` identity, 400 wordings, 404, purge (`delete` role, `?confirm=`). Ladder snapping and cover/gravity/rect geometry run only at `imageFidelity: "full"` (Rust; Worker `RS2_CF_REMOTE`) — local `wrangler dev` emulates width/height/format only |
...
47. **Image transforms are a capability, and the image service stays tenant
    code on both hosts.** The Worker cannot run the Wasm component (§A) and a
    JS bundle cannot move pixels (string bodies), so the platform's Images
    binding does the pixel work — but behind a *grant*, never as a built-in
    service and never as an `env` binding the guest touches. The `images`
    grant is by-reference: the guest names the source path and the cache
    path, the host reads through the sibling `source` grant under the
    caller's principal and writes through the sibling `cache` grant, and no
    image bytes enter the sandbox — the same shape `x-rs2-body-ref` already
    uses for the read side. That keeps the choke points (authz on the
    originals, default-deny grants, the outbound budget) exactly where
    `architecture.md` puts them, keeps the runtime image-free for tenants that
    don't mount it, and lets one tenant file (`code:image@<v>` + the same
    `grants`/`widths`/`caching` config) mount on both hosts with the same
    URLs, params, response headers, strong ETags, `x-img-cache` values and
    `.rs2-store/<root>/d/<kk>/<key>.<ext>` cache layout. The Rust host accepts
    the grant shape and 501s at use rather than 400ing at build, so a config
    is portable even where the backend is not. Cost: a second bundle whose
    param/canonicalization logic duplicates `params.rs` (pinned equal by
    mirrored unit tests), and a mount that needs a deploy before it serves —
    which is already true on Rust. The grant's vocabulary is RS2's, not the
    binding's, so another guest (or a Rust `images` backend later) can use
