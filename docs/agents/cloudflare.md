...
| `message/media_type.rs` | `runtime/media-type.ts` | port | Ordered extension table; `schema=` param round-trips in `Content-Type` (`application/json; schema="…"`); `essence` lowercased. |
...
`Accept-Ranges`, `Last-Modified`, `Vary`, `Cache-Control`, `Allow`,
