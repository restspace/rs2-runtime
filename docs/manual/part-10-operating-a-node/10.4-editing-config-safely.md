...
To promote a tenant from one node to another (staging → production), `rs2 sync
--from A --to B` applies the same `GET → PUT If-Match` round-trip to the target's
config, after deploying any code bundles the config pins and before copying
specs and data. It keeps the target's own `services` mount verbatim, so the
control surface you are talking through never moves or re-locks under you, and
`--mount /p` limits the change to one entry. Preview with `--dry-run`.
